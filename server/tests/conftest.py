...
@pytest.fixture
def make_tasks_module():
    """Reimport the router bound to a fresh FakeDbInstance.

    The router captures ``db_instance`` at import time, so swapping the module
    globals is the honest way to give each test its own recorder.
    """
    ...
    def _make(fail_on=None):
        ...
        import routers.dashboard.tasks as tasks_module

        tasks_module.db_instance = db_instance
        tasks_module.sqloader = StubSqloader()
        return tasks_module, db_instance
